...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.nn(x) * (1 - x[:, -1:]) + \
            torch.log((1 + torch.sum(temp, 1, keepdims=True)) / 2)
...
class PINN:
    ...
    def HJB_Rosenbrock(self): 
        ...
        residual_pred = u_t.reshape(-1) + 0.5 * torch.sum(u_xx, dim=1) - torch.sum(u_x**2, dim=1)
    ...
    def get_loss_pinn_stochastic2_HJB_Rosenbrock(self, bs):
        ...
        for i in idx:
            d2f_dxidxi = torch.autograd.grad(u_x[:, i].sum(), x, create_graph=True)[0][:, i]
            residual_pred += 0.5 * (args.dim - 1) / bs * d2f_dxidxi.detach()
        idx = np.random.choice(self.dim - 1, bs, replace=False)
        for i in idx:
            d2f_dxidxi = torch.autograd.grad(u_x[:, i].sum(), x, create_graph=True)[0][:, i]
            gradient_j += 0.5 * (args.dim - 1) / bs * d2f_dxidxi
